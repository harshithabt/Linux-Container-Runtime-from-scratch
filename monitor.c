...
// TEAM DETAILS:-
// Member 1: Gaurav Shivakumar (PES1UG24AM104)
// Member 2: Harshitha.B.T (PES1UG24AM118)
...
struct monitored_entry {
    ...
    int              soft_warned;   /* 1 = warning already emitted once */
};
...
/*
 * We chose mutex over spinlock because:
 *   - timer_callback and monitor_ioctl both access the list
 *   - timer callbacks run in process context (not hard-IRQ context)
 *     so sleeping inside mutex_lock is allowed
 *   - kmalloc(GFP_KERNEL) in the register path can sleep,
 *     which rules out spinlock entirely
 */
...
static void timer_callback(struct timer_list *t)
{
    ...
    /*
     * list_for_each_entry_safe saves the next pointer before we
     * might free the current entry — safe for deletion during iteration
     */
